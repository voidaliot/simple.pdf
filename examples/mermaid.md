...
Using [Mermaid](https://mermaid-js.github.io/mermaid/#/stateDiagram?id=styling) to render diagrams

## How to render diagrams in markdown documents?
Open the file in a Markdown compatible application, here couple of suggestions:

- Web: You can use StackEdit https://stackedit.io/app# to render these diagrams.
- MacOS/Windows: Recommended [Typora](https://typora.io/)

For example: if you download and open this file using typora you will see what i'm talking about :)

### Sequence Diagram

```mermaid
sequenceDiagram
User->>Web App: request homepage
Web App->>API: fetch page data
API->>Web App: page data
Web App->>User: homepage
Web App-->>API: update user usage
Note right of Web App: User subscription <br>has limits, we must <br>keep usage up to <br>date
API --> SearchOptimizer: Search data
SearchOptimizer -->> Elastic Search: Update indexes
User->>Elastic Search: search data...
Elastic Search->>User: results...
...
```mermaid
graph LR
A(Strapi Headless CMS) --> B[Postgres]
B --> C[Hasura]
C -- GraphQL --> D
B --> A
