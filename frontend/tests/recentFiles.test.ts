...
const entry: RecentEntry = { path: "C:\\diagram.pdf", title: "diagram", lastOpened: 1, pinned: true };

test("recent storage drops removed text formats while preserving PDF pins", () => {
  const removed = ["md", "markdown", "mmd", "mermaid", "puml", "plantuml", "pu", "uml"].map(extension => ({ ...entry, path: `C:\\old.${extension}` }));
  assert.deepEqual(loadRecents(JSON.stringify([...removed, entry])), [{ ...entry, thumbnail: undefined }]);
});
...
test("corrupt or unexpected recent storage cannot crash the home screen", () => {
  ...
  assert.equal(loadRecents(JSON.stringify([entry, { ...entry, path: "c:/DIAGRAM.pdf" }])).length, 1);
...
test("reopening aliases preserves the existing pin and thumbnail", () => {
  const result = addRecent([{ ...entry, thumbnail: "thumb" }], "c:/DIAGRAM.pdf", "renamed", 2);
