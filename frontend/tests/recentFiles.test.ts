...
const entry: RecentEntry = { path: "C:\\diagram.md", title: "diagram", lastOpened: 1, pinned: true };
...
test("corrupt or unexpected recent storage cannot crash the home screen", () => {
  ...
  assert.equal(loadRecents(JSON.stringify([entry, { ...entry, path: "c:/DIAGRAM.md" }])).length, 1);
...
test("reopening aliases preserves the existing pin and thumbnail", () => {
  const result = addRecent([{ ...entry, thumbnail: "thumb" }], "c:/DIAGRAM.md", "renamed", 2);
