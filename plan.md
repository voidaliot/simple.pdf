...
- [x] Form reset support where PDF provides it (FR-FORM-04)
...
- 2026-04-29 — FR-FORM-04: form reset now correct. Added `action_type` field to `FormField` (Rust + TS); classified via button-name heuristic ("submit"/"other"/default "reset"). Added `reset_all_form_fields` Rust method + Tauri command that resets all pages (matching PDF ResetForm semantics). Frontend `handlePushButton` now: skips submit/other buttons (shows "not supported" alert for submit), calls `resetAllFormFields` for reset buttons, then invalidates and reloads form-field cache for all visible pages.
